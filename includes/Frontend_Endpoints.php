...
class Frontend_Endpoints {
	 ...
	 *
	 * Only a request that already carries a valid `customer_key` resolves a
	 * customer. The historic `user` (email) parameter is intentionally no longer
	 * honoured: resolving a customer from a bare email address turned any
	 * visitor's knowledge of that address into an unauthenticated disclosure of
	 * the customer's persistent `customer_key` — the bearer credential for the
	 * communication-preferences surface — straight out of the redirect
	 * `Location` header. An email address is an identifier, not an
	 * authenticator, so it must not resolve (or silently create) a customer
	 * here. Genuine unsubscribe links generated by AutomateWoo carry the
	 * `customer_key`, so the legitimate flow is unaffected.
	 *
	 * @since 4.0.0
	 * @since 6.8.2 The legacy `user` (email) parameter is no longer resolved.
	 */
	public static function catch_legacy_unsubscribe_url() {
		$customer_key = Clean::string( aw_request( 'customer_key' ) );
		$customer     = $customer_key ? Customer_Factory::get_by_key( $customer_key ) : false;
